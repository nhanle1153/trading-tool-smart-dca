...
| TD-0107 | 🐛 Phát hiện qua review độc lập (không phải lookahead): `touch_count()` (`src/tool_d/zone_strength.py:56-61`) — khi `dang_trong_cum=True`, vòng lặp chỉ kiểm `bat_ra`, KHÔNG kiểm `vo_huong_nguoc` — một cú vỡ mạnh xuyên `zone_low`/`zone_high` giữa lúc đang chờ bật ra bị bỏ qua, sau đó bật ra hợp lệ vẫn được tính là 1 touch. Tái hiện: zone day [100,102], giá `[101, 101, 90, 103]` (chạm→vỡ sâu dưới 90→bật lên 103) → `touch_count()` trả 1, không phải 0 | 🔓 | TD-0104 | Test tái hiện đúng kịch bản trên → sau khi sửa phải trả 0 (hoặc giá trị đúng theo định nghĩa LD-35 mà chủ dự án chốt); không phá 22 test hiện có của `test_zone_strength.py` |
