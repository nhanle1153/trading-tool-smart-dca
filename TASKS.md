...
| TD-0119 | **Phần 2 — Context Trend Filter**: `trend_dir`/`trend_dir_4h` (EMA20/50 + slope, §2.1), xác nhận đa khung (§2.2, L-Z8), tuổi trend ≥5 nến (§2.3), điều kiện vào lệnh §2.5 | 🔓 | TD-0104 | Test đơn vị hàm thuần (TA-Lib EMA/ADX) + `tests/lock/test_lz8_*` (đồng thuận 1D/4H tại mọi bản ghi plan) |
| TD-0120 | **§3.3b — Xác nhận entry bằng price-action**: rejection wick (hình học nến) HOẶC RSI(14,1H) divergence dùng lại `touch_count` (TD-0104) — tối đa 3 nến chờ, không nới hạn (L-Z6) | 🔓 | TD-0104, TD-0119 | `tests/lock/test_lz6_*` — mọi tranche 1 khớp đều có `entry_confirmation` không rỗng; hết 3 nến không xác nhận → bỏ lượt, không quay lại tìm |
...
| TD-0122 | **DG7 — Funding Stop**: `funding_paid_cumulative = -trade.funding_fees` (đọc thẳng field native Freqtrade, KHÔNG tự xây pipeline tích luỹ — LD-09) ≥ 0.3×R_eff → đóng MARKET, tag `FUNDING_STOP` (§4c.2) | 🔓 | TD-0112 | `tests/lock/test_lz43_*` — LONG giả lập qua 3 mốc funding dương phải cho `funding_paid_cumulative > 0`; đọc đúng cột `open` của dữ liệu funding (không phải `close`) |
