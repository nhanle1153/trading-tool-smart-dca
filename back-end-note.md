...
| MT-09 | **Trùng tên test khoá `L-Z56`.** Spec đã gán `L-Z56` cho test *"bộ chạy ablation TỪ CHỐI khởi động khi chưa có kết quả cổng D3.5"* (dòng 278 và 3692: *"Test khoá cứng — xem L-Z56 → L-Z58"*). Quyết định **MT-08** (07/09/2026) lại gán `L-Z56` cho test kế toán CTRL (`n_used()` loại CTRL + `reserve()` CTRL vẫn chạy khi ngân sách cạn) — hai test khác hẳn nhau, cùng một mã | Spec dòng 278/3692 vs MT-08 mục (3) | 🟡 Chưa gây thiệt hại (chưa test nào được viết) nhưng phải sửa **trước khi viết test đầu tiên mang mã đó**, nếu không sổ test có hai `L-Z56` và mọi câu "L-Z56 PASS" thành mơ hồ | **CHƯA CHỐT — cần chủ dự án chọn.** Hai hướng: (a) đổi mã test CTRL sang **`L-Z59`** (spec dùng tới L-Z58, L-Z59 còn trống) — rẻ nhất, không đụng spec; (b) giữ `L-Z56` cho CTRL và dời test cổng D3.5 sang mã khác — phải sửa spec ở 2 chỗ, đắt hơn và spec là nguồn sự thật cao nhất (N1). **Nghiêng về (a)**, nhưng đây là mã do phiên khác đặt nên không tự đổi (quy tắc 11: không tự chọn một bên) | 07/09/2026 | *chờ* |
