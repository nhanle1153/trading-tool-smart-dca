"""TD-0255 — bảng R THEO TỪNG LỆNH của một suất: cầu nối giữa E1 và `chon_gia_tri` (TD-0254).

`chon_gia_tri` so hai suất theo khoá `(pair, giờ mở)` (`DR-D5-01` §5). Để làm được việc đó SAU khi các suất đã chạy
xong, mỗi suất phải để lại R của TỪNG lệnh — không chỉ trung bình. E1 ghi file này sau con dấu (`L-Z53`); tầng đánh
giá D5 chỉ đọc file, không chạy lại gì.

Hai thước cạnh nhau, không phán xét (`DR-D4-12` §1.3): `r_trien_khai` (đơn vị phán quyết của `DR-D5-01` §5 — *"R
theo rủi ro đã triển khai"*) và `r_realized` (mẫu số `planned_risk_usdt`). ⚠️ Docstring `chon_gia_tri.py:3` viết
...
class BangRError(ValueError):
    """File bảng R hỏng, trùng khoá, hoặc thước lạ — fail-closed."""
...
def doc_bang_r(duong: Path, *, thuoc: str = "r_trien_khai") -> dict[tuple[str, datetime], float]:
    """`{(pair, giờ mở): R}` — đúng hình `chon_gia_tri` nhận. Trùng khoá ⇒ raise: hai lệnh cùng mã cùng giờ mở là
    ...
    try:
        ...
        raise BangRError(f"{duong}: không đọc được bảng R: {exc}") from exc
