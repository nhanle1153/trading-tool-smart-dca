...
│  │                                  DR-LOCKBOX-01); đời sống mã đo thật (td0306). Commit đầu tiên
│  │                                  = mốc chống đổi rổ của bản cấp lại seal (DR-LOCKBOX-01 §3)
...
│  │                (D9, DR-D9-01 — TD-0281 khoá CSCV đối chiếu khối băm DR,
│  │                TD-0282 PBO thuần, TD-0285 phán quyết D9 + PBO P0)
│  │                · arm_record · ket_cuc · d4_gate · d0_9 (D4 — bản ghi arm, ba kết cục,
│  │                tiêu chí đóng cổng DR-D4-11, gate §10.2 dùng CHUNG ngưỡng với D9 — TD-0341)
│  ├─ bo_chay/      yeu_cau · chay · moi_truong · doc_ket_qua (lõi bộ chạy, DR-BC-01) ·
...
🔄 **Nối thêm 18/09/2026 (`DR-LOCKBOX-02`, TD-0316) — hệ quả chưa ai viết ra của chính việc che này:** H17
(*"kiểm SHA-256 lockbox mỗi lần khởi động pipeline"*) không thể băm dữ liệu ở service mà dữ liệu bị che —
trước TD-0316, E1/E2/E3 thoát 89 MỌI LẦN ở chính service chúng chạy (`MT-63`). Nay H17 tách theo service:

| Service | H17 kiểm gì | Ai gọi |
|---|---|---|
| `freqtrade` / `tests` (che lockbox) | **cách ly còn hiệu lực** (thử đọc thật file lockbox — đọc được ⇒ exit 108) · seal không bị sửa (git) · sổ truy cập | E1/E2/E3 — `lockbox/h17.py` |
| `lockbox` (thấy dữ liệu) | **băm** (`L-Z14`) + **xét rổ** (`MT-60`) | E4 `--verify-seal` — `lockbox/ro_seal.verify_lockbox()` |

Pipeline không bao giờ cần dữ liệu lockbox; lần chạm duy nhất đi qua E4. Việc che giờ được **kiểm lúc
chạy**, không chỉ trong test.
...
- Hàm đó từ chối `LOCKBOX` cho tới khi `MT-60` giải, và **không bao giờ** trả `pool.yaml` cho backtest.
  ...
  mở bảng là gỡ chốt giữa bộ chạy và dữ liệu chỉ chạm một lần. Rổ `T2` cho lần chạm đi đường riêng của E4.
...
| `idea_queue.jsonl` | `idea_queue_entry.schema.json` | `ledger/idea_queue.py` · L-Z16/17 · TD-0119a/b · TD-0120 · TD-0124 · **TD-0326** — từ 19/09/2026 là **nhật ký sự kiện** (`ledger/idea_events.py`, `DR-IQ-02`): mỗi dòng một sự kiện QUEUED/REJECTED/SELECTED/VOIDED/ARCHIVED trên `idea_id` |
