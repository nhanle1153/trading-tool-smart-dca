...
| Kiểm bảo mật tài khoản phụ trước khi bật D10 (`DR-D10-02` Q3, TD-0384 — chưa code) | Binance SAPI **KÝ** — `GET https://api.binance.com/sapi/v1/account/apiRestrictions` | Không có (không đọc được ⇒ TỪ CHỐI bật D10, fail-closed) | JSON cờ quyền của API key: `ipRestrict`, `enableWithdrawals`, `enableInternalTransfer`, `permitsUniversalTransfer`, `enableFutures` (chưa verify bằng gọi thật) | 0 (miễn phí) | weight 1, một lần mỗi lần khởi động | Chỉ Binance biết cấu hình key của chính nó; checklist tay có thể bị mở lại mà không ai hay (`api-integration-rules.md` 4.4c) |
